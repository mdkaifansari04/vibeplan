                      ...
                      .map((segment, index, array) => {
                        return (
                          <Fragment key={segment}>
                            <Link key={segment} href={`/${array.slice(0, index + 1).join("/")}`} title={segment} className="text-nowrap px-1 text-neutral-500 transition-opacity hover:opacity-80 dark:text-neutral-300">
                              {segment}
                            </Link>
                            {index !== array.length - 1 ? <span className="text-neutral-400 dark:text-neutral-600 ">/</span> : null}
                          </Fragment>
                        );
                      })}
                  </>
