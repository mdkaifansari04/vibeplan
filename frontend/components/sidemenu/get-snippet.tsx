...
export async function getSnippetsContent({ category, subcategory, snippet }: { category: string; subcategory: string; snippet: string }) {
  const snippetsDirectory = path.join(process.cwd(), "../../packages/snippets");
  const snippetPath = path.join(snippetsDirectory, category, subcategory, `${snippet}.md`);
