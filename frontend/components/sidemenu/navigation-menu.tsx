...
import { getSidemenuItems } from "./sidemenu-items-list";
import { NavigationSidemenuAnimatedBackground } from "./navigation-animated-background";
import { cn } from "@/lib/utils";

export default async function NavigationMenu({ section, isMobile, className }: { section: string; isMobile?: boolean; className?: string }) {
        ...
        <NavigationSidemenuAnimatedBackground>
          {sidemenuContent.items.map((item) => (
            <li key={item.name} className="block" data-id={item.name}>
              <GlobalNavItem Icon={item.Icon} href={item.href} isMobile={isMobile} name={item.name} tag={item.tag} target="sameWindow" />
            </li>
          ))}
        </NavigationSidemenuAnimatedBackground>
      </nav>
