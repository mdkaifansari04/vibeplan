...
export type SizePresets = "reset" | "empty" | "default" | "compact" | "compactLong" | "large" | "long" | "minimalLeading" | "minimalTrailing" | "compactMedium" | "medium" | "tall" | "ultra" | "massive";
...
const DynamicIslandProvider: React.FC<DynamicIslandProviderProps> = ({ children, initialSize = SIZE_PRESETS.DEFAULT, initialAnimation = [] }) => {
...
const DynamicIslandContent = ({ children, id, willChange, screenSize, ...props }: { children: React.ReactNode; id: string; willChange: any; screenSize: string; [key: string]: any }) => {
...
const DynamicContainer = ({ className, children }: DynamicContainerProps) => {
  ...
  return (
    <motion.div initial={initialState} animate={animateState} transition={transition} exit={{ opacity: 0, filter: "blur(10px)", scale: 0.95, y: 20 }} style={{ willChange }} className={className}>
      {children}
    </motion.div>
  );
};
...
export { DynamicContainer, DynamicTitle, DynamicDescription, DynamicIsland, SIZE_PRESETS, stiffness, DynamicDiv, damping, DynamicIslandSizePresets, BlobContext, useDynamicIslandSize, useScheduledAnimations, DynamicIslandProvider };
