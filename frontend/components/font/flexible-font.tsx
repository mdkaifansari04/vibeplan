...
export const VariableFontCursorProximity = ({ children, fromFontVariationSettings = "'wght' 400, 'slnt' 0", toFontVariationSettings = "'wght' 900, 'slnt' -10", containerRef, radiusZoomingZone = 50, falloff = "linear", className, onClick, ref, ...props }: TextProps & ComponentProps<"span">) => {
  ...
  const calculateFalloff = (distance: number): number => {
    const normalizedDistance = Math.min(Math.max(1 - distance / radiusZoomingZone, 0), 1);
  ...
  useAnimationFrame(() => {
    ...
    letterRefs.current.forEach((letterRef, index) => {
      ...
      const distance = calculateDistance(mousePosition.elementX, mousePosition.elementY, letterCenterX, letterCenterY);
