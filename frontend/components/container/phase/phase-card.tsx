  ...
  return (
    <div className="flex items-start gap-2 rounded-md border border-border bg-card/80 px-2 py-2">
      ...
      <button onClick={onRegenerate} className="ml-auto inline-flex h-6 w-6 items-center justify-center rounded-md border border-border text-foreground/70 hover:bg-accent hover:text-foreground transition" aria-label="Regenerate plan" title="Regenerate plan">
        {/* rotate-cw icon (inline svg), size sm */}
        <svg width="14" height="14" viewBox="0 0 24 24" className="opacity-80" fill="currentColor" aria-hidden="true">
          <path d="M12 6a6 6 0 1 1-5.657 8H3a1 1 0 0 1 0-2h3.343A6 6 0 0 1 12 6Zm-1-4a1 1 0 0 1 1-1 11 11 0 0 1 9.9 6H23a1 1 0 1 1 0 2h-5a1 1 0 0 1-1-1V3a1 1 0 1 1 2 0v1.315A9 9 0 0 0 12 2a1 1 0 0 1-1-1Z" />
        </svg>
      </button>
    </div>
  );
...
export function PhaseCard({ phase, plans = [], onGeneratePlan, onOpenPlan }: { phase: Phase; plans?: Plan[]; onGeneratePlan?: (phaseId: string) => void; onOpenPlan?: (phaseId: string, index: number) => void }) {
  // Header content
  const topContent = (
  ...
  // Image that shrinks into the header on hover
  const topAnimateContent = (
    <>
      <motion.img transition={{ duration: 0.3, ease: "circIn" }} src="/phase-thumbnail.jpg" layoutId={`phase-img-${phase.id}`} width={78} height={100} alt="Phase thumbnail" className="absolute right-2 top-1.5 rounded-sm shadow-lg" />
      <motion.div
  ...
  // The main image in the middle
  const middleContent = <motion.img src="/phase-image.jpg" layoutId={`phase-img-${phase.id}`} width={150} height={200} alt="Phase preview" className="rounded-sm border-2 border-white dark:border-black" />;

  // Footer content that appears on hover
  const bottomContent = (
    <div className="pb-3">
